...
const covenantArgs = id => [id, "eaglebooth/ForkRight", accounts[1].address, standard, 120n, 30n, 60n];
...
// Deterministic failure paths first.
await write("failure.selfSteward", "register_covenant", [`bad-${tag}`, "eaglebooth/ForkRight", accounts[0].address, standard, 120n, 30n, 60n], maintainer, "INVALID_COVENANT");
const activeCov = `active-${tag}`; const activeClaim = `claim-active-${tag}`;
await write("active.register", "register_covenant", covenantArgs(activeCov), maintainer);
await write("failure.duplicateCovenant", "register_covenant", covenantArgs(activeCov), maintainer, "INVALID_OR_DUPLICATE_COVENANT");
await write("active.open", "open_claim", claimArgs(activeClaim, activeCov, "active"), steward);
await write("active.assess", "assess_claim", [activeClaim], steward);
const activeState = await read("get_claim", [activeClaim]);
if (activeState.status !== "DISMISSED" || activeState.verdict !== "ACTIVE") throw new Error(`Expected ACTIVE/DISMISSED, got ${JSON.stringify(activeState)}`);
...
const successionCov = `succession-${tag}`; const successionClaim = `claim-succession-${tag}`;
await write("succession.register", "register_covenant", covenantArgs(successionCov), maintainer);
await write("succession.open", "open_claim", claimArgs(successionClaim, successionCov, "abandoned"), steward);
await write("succession.assess", "assess_claim", [successionClaim], steward);
const pending = await read("get_claim", [successionClaim]);
...
await write("succession.finalize", "finalize_succession", [successionClaim], steward);
const successionState = await read("get_covenant", [successionCov]);
if (successionState.state !== "SUCCESSOR_RECOGNIZED") throw new Error("Succession invariant failed");
const stats = await read("get_stats");
process.stdout.write(`FORKRIGHT_LIVE_COMPLETE ${JSON.stringify({ contract, evidenceCommit, wallets: { maintainer: accounts[0].address, steward: accounts[1].address }, activeState, restoreState, successionState, stats, transactions }, null, 2)}\n`);
